...
def _cluster_into_bins(eval_data, ref_data, classifier, num_clusters, maskset):
    ...
    #TODO: Replace `MiniBatchKMeans` here the svm.OneClassSVM(). Load it using joblib.load(clf, "saved_model.pkl").
    #clf = joblib.load(f"./trained_models/etest_clf_maskset_{maskset}.pkl")
    #labels = clf.predict(cluster_data)
...
def compute_prd_from_embedding(eval_data, ref_data, classifier, num_clusters=20,
                               num_angles=1001, num_runs=10,
                               enforce_balance=True, maskset=0):
    ...
    precisions = []
    recalls = []
    eval_dist, ref_dist = _cluster_into_bins(eval_data, ref_data, classifier, num_clusters, maskset)
