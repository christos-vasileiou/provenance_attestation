...
class GeneratorInline(nn.Module):
    def __init__(self, hps):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d(249, 64 * 8, kernel_size=4, stride=1, padding=0),
            nn.BatchNorm2d(64 * 8),
            # intermediate: 2X2X[64*8]
            nn.ConvTranspose2d(64 * 8, 64 * 4, kernel_size=4, stride=1, padding=0),
            nn.BatchNorm2d(64 * 4),
            # intermediate: 4X4X[64*4]
            nn.ConvTranspose2d(64 * 4, 1, kernel_size=4, stride=1, padding=0),
            nn.BatchNorm2d(1),
...
class Discriminator(nn.Module):
    def __init__(self, hps):
        ...
        self.discriminator_layer_0 = nn.Sequential(
            # input is (nc) x 13
            nn.Conv1d(hps['nc'], hps['ndf'], 3, stride=2, padding=1),
            nn.LeakyReLU(0.2, inplace=True)
        )
        self.discriminator_layer_1 = nn.Sequential(
            # state size. (ndf) x 7
            nn.Conv1d(hps['ndf'], hps['ndf'] * 2, 3, stride=2, padding=1),
            nn.BatchNorm1d(hps['ndf'] * 2),
            nn.LeakyReLU(0.2, inplace=True)
        )
        self.discriminator_layer_2 = nn.Sequential(
            # state size. (ndf*2) x 4
            nn.Conv1d(hps['ndf'] * 2, hps['ndf'] * 4, 3, stride=2, padding=1),
            nn.BatchNorm1d(hps['ndf'] * 4),
            nn.LeakyReLU(0.2, inplace=True)
        )
        self.discriminator_layer_3 = nn.Sequential(
            # state size. (ndf*2) x 1
            nn.Conv1d(hps['ndf'] * 4, 1, 2, stride=1, padding=0)
        )
