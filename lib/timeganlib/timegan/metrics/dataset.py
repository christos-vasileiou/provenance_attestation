...
class OneStepPredictionDataset(torch.utils.data.Dataset):
    ...
    - data (np.ndarray): the dataset to be trained on (B x S x F)
    """
    def __init__(self, data, time, data_path=str()):
        max_seq_len = 100 if 'multiclass_dataset_8181_254.csv' not in data_path else 27
        self.X = torch.FloatTensor(data[:, :-1, :])
        self.T = torch.LongTensor([t-1 if t == max_seq_len else t for t in time])
