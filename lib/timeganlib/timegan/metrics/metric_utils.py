...
from timegan.metrics.general_rnn import GeneralRNN
from timegan.metrics.dataset import FeaturePredictionDataset, OneStepPredictionDataset
...
def feature_prediction(train_data, test_data, index, max_seq_len=100):
    ...
    for idx in index:
        ...
        for epoch in logger:
            ...
            for train_x, train_t, train_y in train_dataloader:
                ...
                #print(f"train_p: {train_p.shape}, train_y: {train_y.shape}")
...
def one_step_ahead_prediction(train_data, test_data, data_path=str()):
    ...
    args["epochs"] = 100
    args["batch_size"] = 128
    args["in_dim"] = dim  if 'multiclass_dataset_8181_254.csv' not in data_path else  dim
    args["h_dim"] = dim   if 'multiclass_dataset_8181_254.csv' not in data_path else  dim
    args["out_dim"] = dim if 'multiclass_dataset_8181_254.csv' not in data_path else  dim
    args["n_layers"] = 3
    args["dropout"] = 0.5
    args["padding_value"] = -1.0
    args["max_seq_len"] = 100 - 1 if 'multiclass_dataset_8181_254.csv' not in data_path else 27-1 # only 99 is used for prediction
    args["learning_rate"] = 1e-3
    args["grad_clip_norm"] = 5.0

    # Set training features and labels
    train_dataset = OneStepPredictionDataset(train_data, train_time, data_path=data_path)
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        batch_size=args["batch_size"],
        shuffle=True
    ...
    # Set testing features and labels
    test_dataset = OneStepPredictionDataset(test_data, test_time, data_path=data_path)
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset,
        batch_size=no,
        shuffle=True
