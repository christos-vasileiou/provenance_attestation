...
class TimeGAN(torch.nn.Module):
    ...
    def _recovery_forward(self, X, T):
        ...
        #print(f"H: {H.shape}, X_tilde: {X_tilde.shape}")

        # For Joint training
        H_hat_supervise = self.supervisor(H, T)
        #print(f"H_hat_supervise: {H_hat_supervise.shape}")
        #print(f"H_hat_supervise[:, :-1, :]: {H_hat_supervise[:, :-1, :].shape}, H[:, 1:, :]: {H[:, 1:, :].shape}")
        ...
        #print(f"E_loss: {E_loss}, E_loss0: {E_loss0}, E_loss_T0: {E_loss_T0}, G_loss_S: {G_loss_S}")
