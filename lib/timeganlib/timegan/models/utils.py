...
from timegan.models.dataset import TimeGANDataset
...
def embedding_trainer(
    ...
    """
    logger = trange(args.emb_epochs, desc=f"Epoch: 0, Loss: 0", position=3)
    for epoch in logger:
...
def supervisor_trainer(
    ...
    """
    logger = trange(args.sup_epochs, desc=f"Epoch: 0, Loss: 0", position=4)
    for epoch in logger:
        for X_mb, T_mb in dataloader:
            # Reset gradients
            model.zero_grad()
...
def timegan_trainer(model, data, time, args):
    ...
    args.model_path = os.getcwd()+"/output"
